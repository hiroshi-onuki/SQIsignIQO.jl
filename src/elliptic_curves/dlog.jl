...
function ec_dlog_power_of_2_E0(P::Point{T}, Q::Point{T}, cdata::CurveData) where T <: RingElem
    A0 = cdata.A0
...
# return n1, n2, n3, n4 such that P = [n1]P0 + [n1]Q0, Q = [n3]P0 + [n4]Q0, where (P0, Q0) is a fixed basis of E0[2^ExponentFull]
function ec_dlog_power_of_2_E0(xP::Proj1{T}, xQ::Proj1{T}, xPQ::Proj1{T}, cdata::CurveData) where T <: RingElem
    A0 = cdata.A0
    P = Point(A0, xP)
    Q = Point(A0, xQ)
    PQ = add(P, -Q, Proj1(A0))
    ...
    return ec_dlog_power_of_2_E0(P, Q, cdata)
end
